...
type IndexProps = {
    readonly projects: PaginatedData<ProjectRow>;
    readonly filters: { readonly q?: string; readonly status?: string };
};
...
function patch(id: number, action: string) {
    router.patch(
        `/admin/projects/${id}/${action}`,
        {},
        { preserveScroll: true },
    );
}

export default function ProjectsIndex({ projects, filters }: IndexProps) {
    const [search, setSearch] = useState(filters.q ?? '');
    const [statusFilter, setStatusFilter] = useState(filters.status ?? '');

    const title = (project: ProjectRow): string =>
        project.title?.es ?? project.title?.en ?? '—';
    const slug = (project: ProjectRow): string =>
        project.slug?.es ?? project.slug?.en ?? '';
            ...
            <div className="rounded-2xl border border-qd-mist bg-qd-white dark:border-qd-white/10 dark:bg-qd-surface">
                ...
                {projects.data.length === 0 ? (
                    ...
                    <div className="overflow-x-auto">
                                            ...
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        >
                                                            {project.showInCollaborations
                                                                ? 'Ocultar de Colaboraciones'
                                                                : 'Mostrar en Colaboraciones'}
                                                        </DropdownMenuItem>
                                                        <DropdownMenuSeparator />
                                                        <DropdownMenuItem
                                                            variant="destructive"
                        ...
                        <AdminPagination pagination={projects} />
                    </div>
                )}
            </div>
        </AdminLayout>
