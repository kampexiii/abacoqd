...
type IndexProps = { readonly faqs: readonly FaqRow[] };

function patch(id: number, action: string) {
    router.patch(`/admin/faqs/${id}/${action}`, {}, { preserveScroll: true });
}

export default function FaqsIndex({ faqs }: IndexProps) {
    ...
    const filtered = useMemo(() => {
        ...
        return faqs.filter((faq) => `${faq.question?.es ?? ''} ${faq.category ?? ''} ${faq.intent ?? ''}`.toLowerCase().includes(term));
    }, [faqs, search]);

    return (
        <AdminLayout
            title="FAQs / Chatbot"
            breadcrumbs={[{ title: 'Dashboard', href: '/admin/dashboard' }, { title: 'FAQs' }]}
            actions={
                <Link
            ...
            <div className="rounded-2xl border border-qd-mist bg-qd-white dark:border-qd-white/10 dark:bg-qd-surface">
                ...
                {filtered.length === 0 ? (
                    <div className="p-6">
                        <AdminEmptyState
                            icon={HelpCircle}
                            title={faqs.length === 0 ? 'Aún no hay FAQs' : 'Sin resultados'}
                            description={faqs.length === 0 ? 'Crea la primera pregunta frecuente.' : 'Ajusta la búsqueda.'}
                            action={
                                faqs.length === 0 ? (
                                    <Link
                                        href="/admin/faqs/create"
                        ...
                        <table className="w-full min-w-200 text-sm">
                            <thead>
                                <tr className="border-b border-qd-mist text-left text-xs font-semibold tracking-wide text-qd-text-medium uppercase dark:border-qd-white/10 dark:text-qd-white/50">
                                    ...
                                    <th className="px-4 py-3">Categoría / Intent</th>
                                    <th className="px-4 py-3 text-center">Chatbot</th>
                                    <th className="px-4 py-3 text-center">Página FAQs</th>
                            ...
                            <tbody className="divide-y divide-qd-mist dark:divide-qd-white/10">
                                {filtered.map((faq) => (
                                    <tr key={faq.id} className="align-middle">
                                        <td className="px-4 py-3 font-semibold text-qd-ink dark:text-qd-white">{faq.question?.es ?? '—'}</td>
                                        <td className="px-4 py-3 text-qd-text-high dark:text-qd-white/70">
                                            {faq.category ?? '—'}
                                            {faq.intent && <span className="ml-1 text-xs text-qd-text-medium dark:text-qd-white/40">({faq.intent})</span>}
                                        </td>
                                        <td className="px-4 py-3 text-center">
                                            <RowToggle checked={faq.showInChatbot} onClick={() => patch(faq.id, 'toggle-chatbot')} />
                                        </td>
                                        <td className="px-4 py-3 text-center">
                                            <RowToggle checked={faq.showOnPage} onClick={() => patch(faq.id, 'toggle-page')} />
                                        </td>
                                            ...
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        <DropdownMenuItem onSelect={() => patch(faq.id, 'toggle-active')}>
                                                        ...
                                                        <DropdownMenuItem
                                                            variant="destructive"
                                                            onSelect={() => {
                                                                if (window.confirm('¿Archivar esta FAQ?')) {
