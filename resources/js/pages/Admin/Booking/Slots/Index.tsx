...
type IndexProps = {
    readonly slots: PaginatedData<SlotRow>;
...
export default function BookingSlotsIndex({
    slots,
    days,
    filterDayId,
}: IndexProps) {
            ...
            <div className="mb-4 flex items-center gap-2">
                <AdminSelect
                    ...
                    onChange={(e) =>
                        router.get(
                            '/admin/booking/slots',
                            e.target.value ? { day: e.target.value } : {},
                        )
                    }
                    className="w-auto"
                >
                ...
                {slots.data.length === 0 ? (
                    <div className="p-6">
                        <AdminEmptyState
                            icon={Clock}
                            title="Sin franjas"
                            description="Crea franjas horarias para un día disponible."
                        />
                    </div>
                ) : (
                    <div className="overflow-x-auto">
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        ...
                                                        <DropdownMenuItem
                                                            ...
                                                            onSelect={() => {
                                                                if (
                                                                    window.confirm(
                                                                        '¿Eliminar esta franja? Solo es posible sin reservas activas.',
                                                                    )
                                                                ) {
                                                                    router.delete(
                                                                        `/admin/booking/slots/${slot.id}`,
                                                                        {
                                                                            preserveScroll: true,
                                                                        },
                                                                    );
                        ...
                        <AdminPagination pagination={slots} />
                    </div>
                )}
            </div>
        </AdminLayout>
