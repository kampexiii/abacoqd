...
type IndexProps = { readonly days: readonly DayRow[] };

function patch(id: number, action: string) {
    router.patch(`/admin/booking/days/${id}/${action}`, {}, { preserveScroll: true });
}
...
export default function BookingDaysIndex({ days }: IndexProps) {
    ...
    return (
        <AdminLayout
            ...
            breadcrumbs={[{ title: 'Dashboard', href: '/admin/dashboard' }, { title: 'Reserva' }, { title: 'Días' }]}
            actions={
                <Link
        ...
        >
            ...
            <div className="rounded-2xl border border-qd-mist bg-qd-white dark:border-qd-white/10 dark:bg-qd-surface">
                {days.length === 0 ? (
                    <div className="p-6">
                        <AdminEmptyState icon={CalendarDays} title="Aún no hay días configurados" description="Crea el primer día disponible para reservas." />
                    </div>
                ) : (
                        ...
                        <table className="w-full min-w-180 text-sm">
                            <thead>
                                <tr className="border-b border-qd-mist text-left text-xs font-semibold tracking-wide text-qd-text-medium uppercase dark:border-qd-white/10 dark:text-qd-white/50">
                                    ...
                                    <th className="px-4 py-3 text-center">Franjas</th>
                                    <th className="px-4 py-3 text-center">Disponible</th>
                                    <th className="px-4 py-3 text-center">Bloqueado</th>
                            ...
                            <tbody className="divide-y divide-qd-mist dark:divide-qd-white/10">
                                {days.map((day) => (
                                    <tr key={day.id}>
                                        <td className="px-4 py-3 font-semibold text-qd-ink dark:text-qd-white">{day.date}</td>
                                        <td className="px-4 py-3 text-qd-text-high dark:text-qd-white/70">{day.title ?? '—'}</td>
                                        <td className="px-4 py-3 text-center">
                                            <Link href={`/admin/booking/slots?day=${day.id}`} className="text-qd-teal-2 hover:underline dark:text-qd-teal">
                                                {day.slotsCount}
                                            </Link>
                                        </td>
                                        <td className="px-4 py-3 text-center">
                                            <RowToggle checked={day.isAvailable} onClick={() => patch(day.id, 'toggle-available')} />
                                        </td>
                                        <td className="px-4 py-3 text-center">
                                            <RowToggle checked={day.adminBlocked} onClick={() => patch(day.id, 'toggle-blocked')} />
                                        </td>
                                                ...
                                                <DropdownMenu>
                                                    ...
                                                    <DropdownMenuContent align="end">
                                                        <DropdownMenuItem
                                                            variant="destructive"
                                                            onSelect={() => {
                                                                if (window.confirm('¿Eliminar este día? Solo es posible si no tiene reservas activas.')) {
                                                                    router.delete(`/admin/booking/days/${day.id}`, { preserveScroll: true });
                                                                }
                                                            }}
                                                        >
                                                            Eliminar
