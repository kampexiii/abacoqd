...
type IndexProps = { readonly bookings: readonly BookingRow[] };
...
export default function BookingBookingsIndex({ bookings }: IndexProps) {
    ...
    return (
        <AdminLayout title="Reservas" breadcrumbs={[{ title: 'Dashboard', href: '/admin/dashboard' }, { title: 'Reserva' }, { title: 'Reservas' }]}>
            ...
            <div className="rounded-2xl border border-qd-mist bg-qd-white dark:border-qd-white/10 dark:bg-qd-surface">
                {bookings.length === 0 ? (
                    <div className="p-6">
                        <AdminEmptyState icon={NotebookPen} title="Aún no hay reservas" description="Las reservas confirmadas desde /reserva aparecerán aquí." />
                    </div>
                ) : (
                                ...
                                {bookings.map((booking) => (
                                    <tr key={booking.id}>
                                        <td className="px-4 py-3">
                                            <p className="font-semibold text-qd-ink dark:text-qd-white">{booking.name}</p>
                                            <p className="text-xs text-qd-text-medium dark:text-qd-white/40">{booking.email}</p>
                                        </td>
                                        <td className="px-4 py-3 text-qd-text-high dark:text-qd-white/70">{booking.serviceName ?? '—'}</td>
                                        ...
                                        <td className="px-4 py-3">
                                            <span className={cn('rounded-full px-2.5 py-1 text-xs font-semibold', STATUS_CLASS[booking.status] ?? 'bg-qd-mist dark:bg-qd-white/10')}>
