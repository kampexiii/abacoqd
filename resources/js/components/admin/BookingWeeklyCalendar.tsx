...
const STATE_STYLES: Record<CalendarSlot['state'], string> = {
    ...
    reserved:
        'border-qd-teal-2/40 bg-qd-teal-2/10 text-qd-teal-2 dark:border-qd-teal/30 dark:bg-qd-teal/10 dark:text-qd-teal',
    blocked:
...
function toggleBlocked(slot: CalendarSlot) {
    router.patch(
        `/admin/booking/slots/${slot.id}/toggle-blocked`,
        {},
        { preserveScroll: true },
    );
}
...
function deleteSlot(slot: CalendarSlot) {
    if (slot.activeBookingsCount > 0) {
        window.alert(
            'No se puede eliminar: esta franja tiene una reserva activa.',
        );
    ...
    if (window.confirm('¿Eliminar esta franja?')) {
        router.delete(`/admin/booking/slots/${slot.id}`, {
            preserveScroll: true,
        });
    }
}

export default function BookingWeeklyCalendar({
    days,
}: {
            ...
            {days.map((day) => (
                <div
                    ...
                    className={cn(
                        ...
                        day.isToday
                            ? 'border-qd-teal-2 dark:border-qd-teal'
                ...
                >
                    ...
                    {day.slots.length === 0 ? (
                        <p className="px-1 py-3 text-xs text-qd-text-medium dark:text-qd-white/30">
                            Sin franjas
                        </p>
                    ) : (
                        <ul className="flex flex-col gap-1.5">
                            {day.slots.map((slot) => (
                                <li
                                    key={slot.id}
                                    className={cn(
                                        'group rounded-lg border px-2 py-1.5 text-xs',
